...
namespace Master.Modbus
{
    public class ModbusRTU
    {
        ...
        {
            return (ushort)(bf[n] << 8 | bf[n + 1]);
        }

        public int WrWord(ushort val, byte[] bf, int n)
        {
            bf[n++] = (byte)(val >> 8);
            bf[n++] = (byte)val;
        ...
        public ushort Crc(byte[] bf, int len)
        {
        ...
        public ushort RdCrc(byte[] bf, int n)
        {
            return (ushort)(bf[n + 1] << 8 | bf[n]);
        }

        public int WrCrc(ushort crc, byte[] bf, int n)
        {
            bf[n++] = (byte)crc;
            bf[n++] = (byte)(crc >> 8);
            return n;
        ...
        public int Wr(byte adr, byte fce, ushort reg, int nbr, byte[] vals, byte[] bf)
        {
            ...
            bf[n++] = (byte)bytes;
            for (int i = 0; i < bytes; i++) bf[n++] = vals[i];
            return n;
        }

        public int AnsErr(byte adr, byte fce, byte err, byte[] bf)
        {
            bf[0] = adr;
            bf[1] = fce;
            bf[2] = err;
