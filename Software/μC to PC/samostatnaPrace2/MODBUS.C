#include <8051.h>
#include "modbus.h"

#define _ASCII_
#define _RTU_

word RdWord(byte * bf)
{
  return ( *bf<<8 | *(bf+1));
}

byte WrWord(word val,byte *bf)
{
	*bf++ = val>>8;
	*bf   = val;
	return 2;

}


#ifdef _ASCII_

byte AHex(byte c)
{
  if ((c>=(byte)'0') && (c<=(byte)'9')) return (byte)(c-'0');
  if ((c>=(byte)'A') && (c<=(byte)'F')) return (byte)(c-'A'+10);
  return 0xFF;
}

byte HexAsc(byte b)
{
  if (b<10)  return (byte)(b+'0');
  return (byte)(b-10+'A');
}


byte MbRdByte(byte *bf)
{
	return ((AHex(*bf) << 4) | AHex(*(bf+1)));
}
word MbRdWord(byte *bf)
{
  return ((AHex(*bf) << 12) | (AHex(*(bf+1)) << 8) | (AHex(*(bf+2)) << 4) | AHex(*(bf+3)));
}



byte MbWrByte(byte b,byte *bf)
{
  *bf++ = HexAsc(b >> 4);
  *bf = HexAsc(b & 0xF);
  return 2;
}


byte MbWrWord(word w,byte *bf)
{
		 *bf++ = HexAsc(w >> 12);
		 *bf++ = HexAsc((w >> 8) & 0xF);
		 *bf++ = HexAsc((w >> 4) & 0xF);
		 *bf = HexAsc(w & 0xF);
		 return 4;
}

byte MbWrEoT(byte *bf)
{
  *bf++='\r';
  *bf='\n';
  return 2;
}

byte MbLrc(byte *bf,byte len)
{
  byte lrc=0;

  while(len--) lrc+=*bf++;
  return -lrc;
}


byte MbRd(byte adr,byte fce,word reg,word nbr,byte *bf)
{
  byte n=1;

		 *bf = ':';
		 n += MbWrByte(adr,bf+n);
		 n += MbWrByte(fce,bf+n);
		 n += MbWrWord(reg,bf+n);
		 n += MbWrWord(nbr,bf+n);
		 return n;
}

byte MbWrOne(byte adr,byte fce,word reg,word val,byte *bf)
{
  byte n=1;

		 *bf = ':';
		 n += MbWrByte(adr,bf+n);
		 n += MbWrByte(fce,bf+n);
		 n += MbWrWord(reg,bf+n);
		 n += MbWrWord(val,bf+n);
		 return n;
}


byte MbWr(byte adr,byte fce,word reg,word nbr,byte *vals,byte *bf)
{
  byte n=1, bytes;
		 *bf = ':';
		 n += MbWrByte(adr,bf+n);
		 n += MbWrByte(fce,bf+n);
		 n += MbWrWord(reg,bf+n);
		 n += MbWrWord(nbr,bf+n);
		 switch(fce)
		 {
		   case 0x10: bytes = 2*nbr;break;
		   case 0x0F: bytes = (nbr+7)/8;break;
		   default  : bytes = 0;
		 }
		 n += MbWrByte(bytes,bf+n);
		 while(bytes--)
			n += MbWrByte(*vals++,bf+n);
		 return n;
}


byte MbAnsRd(byte adr, byte fce, byte bytes, byte *vals,byte *bf)
{
  byte n=1;
		 *bf = ':';
		 n += MbWrByte(adr,bf+n);
		 n += MbWrByte(fce,bf+n);
		 n += MbWrByte(bytes,bf+n);
		 while(bytes--)
			n += MbWrByte(*vals++,bf+n);
		 return n;
}


byte MbAnsErr(byte adr,byte fce,byte er,byte *bf)
{
   byte n=1;

		 *bf = ':';
		 n += MbWrByte(adr,bf+n);
		 n += MbWrByte(fce,bf+n);
		 n += MbWrByte(er,bf+n);
		 return n;
}

#endif

#ifdef _RTU_

const byte tabCRCHi[] = {

};

const byte  tabCRCLo[] = {

};


word MrtuCrc(byte * bf, byte len )
{
  byte CRCHi = 0xFF ;  /* MSB CRC */
  byte CRCLo = 0xFF ;  /* LSB CRC  */
  byte index ;         /* index pro tabulku */
  while (len--)
  {
	index = CRCLo ^ *bf++ ;
	CRCLo = CRCHi ^ tabCRCHi[index] ;
	CRCHi = tabCRCLo[index] ;
  }
  len=sizeof(tabCRCLo)-1;
  return (CRCHi << 8 | CRCLo) ;
}

word MrtuRdCrc(byte * bf)
{
  return ( *(bf+1)<<8 | *bf);
}

byte MrtuWrCrc(word crc,byte * bf)
{
	*bf++ = crc;
	*bf   = crc>>8;
	return 2;

}


byte MrtuAnsErr(byte adr,byte fce,byte err,byte *bf)
{
  *bf++=adr;
  *bf++=fce;
  *bf++=err;
  return 3;
}
byte MrtuAnsRd(byte adr,byte fce,byte bytes,byte *vals,byte *bf)
{
   byte n=bytes+3;
  *bf++=adr;
  *bf++=fce;
  *bf++=bytes;
   while(bytes--) *bf++ = *vals++;
   return n;
}
byte MrtuWrOne(byte adr,byte fce,word reg,word val,byte *bf)
{
  *bf++=adr;
  *bf++=fce;
  *bf++=reg>>8;
  *bf++=reg;
  *bf++=val>>8;
  *bf++=val;
   return 6;
}

byte MrtuRd(byte adr,byte fce,word reg,word nbr,byte *bf)
{
  *bf++=adr;
  *bf++=fce;
  *bf++=reg>>8;
  *bf++=reg;
  *bf++=nbr>>8;
  *bf++=nbr;
   return 6;
}

byte MrtuWr(byte adr,byte fce,word reg,word nbr,byte *vals,byte *bf)
{
  byte n,bytes;

  *bf++=adr;
  *bf++=fce;
  *bf++=reg>>8;
  *bf++=reg;
  *bf++=nbr>>8;
  *bf++=nbr;
  switch(fce)
  {
	case 16: bytes=2*nbr;break;
	case 15: bytes= (nbr+7)/8;break;
	default: bytes=0;
  }
  *bf++=n=bytes;
  while(n--) *bf++= *vals++;
  return bytes+7;
}
#endif
