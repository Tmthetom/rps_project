				...
				if ((MbLrc(bufferIn + 1, index - 4) == (lrc = MbRdByte(bufferIn + index - 3))) && (MbRdByte(bufferIn + 1) == ADR_S))
				{
					...
					switch (registerCode)
					{
						...
						case FCE_WREG:
							...
							// Check validity
							if ((reg = MbRdWord(bufferIn + 5)) != REG_WR) errorCode = 2;
							...
							// Prepare answer
							if (errorCode == 0) itx = MbAnsWr(ADR_S, registerCode, reg, val, bufferOut);
							break;
						...
						case FCE_RBIT:
							...
							// Send button state
							if ((reg = MbRdWord(bufferIn + 5)) == BIT_RD && (pocet = MbRdWord(bufferIn + 9)) == 1)
							{
								bity[0] = TLAC;
